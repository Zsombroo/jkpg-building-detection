...
if __name__ == '__main__':
    ...
    for image_path in glob.glob(input_folder+'/*.png'):
        ...
        # Winning preprocessing: Raw + overlay (canny edge (150, 255) + denoised)
        # Normalize

        # Canny edge
        img_canny = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)
        edges = cv2.Canny(img_canny, 150, 255)
        ...
        # Combine raw with edges
        image_np = load_image_into_numpy_array(image_path)

        edge = img.copy()
        edge = cv2.cvtColor(edge, cv2.COLOR_GRAY2BGR)
        combined = cv2.add(image_np, edge)
        
        output_dict = run_inference_for_single_image(model, combined)
